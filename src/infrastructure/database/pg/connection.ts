...
import { InvoiceRepository, INVOICE_SCHEMA, INVOICE_LINES_SCHEMA } from './Invoice';
...
export class PostgresConnection implements IDatabaseConnection {
  ...
  private async runMigrations(): Promise<void> {
    ...
    try {
      await client.query(INVOICE_SCHEMA);
      await client.query(INVOICE_LINES_SCHEMA);
  ...
  public getRepositories(): { invoice: InvoiceRepository } {
    return {
      invoice: new InvoiceRepository(this.getClient()),
    };
  }
}
